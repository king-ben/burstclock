...
In the simulation study we simulated data with varying burst size $b$. Negative values of $b$ result in a model without bursts, and the absolute value of $b$ does not affect the data. Positive values of $b$ result in a Burst Clock model, and the absolute value of $b$ should be identifiable. \cref{f:simulation} shows the simulated and estimated root height, clock rate and burst size for one reconstruction without bursts (left column; $b$ is fixed to $0$) and one reconstruction with bursts (right column; $b$ is estimated).
...
historical linguistics. It would thus contribute to the dialog between the disciplines and to the
