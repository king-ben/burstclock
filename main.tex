...
\begin{figure}
    \centering
     ...
     \node at (3, 1) {$e_i = c \cdot l_i + b$};
    \end{tikzpicture}
    \caption{The burst clock model: For $b>0$, the amount of change (symbolized
      by $\bullet$s) that is
      accumulated from the root $r$ to a node $i$ depends on the number of
      ancestral nodes between $r$ and $i$}
    \label{f:burstclock}
\end{figure}
...
\paragraph{Calibrations}
Following the evidence presented by \textcite[Tables 7 and 12]{chang2015ancestryconstrained}, we implement the following node and tip calibrations, which are minimum ages (capped at an upper bound of 20000~yBP using a uniform prior to obtain a proper prior distribution) or normal distributions as given by the reference. Because of the presence of several old languages, \cref{t:indoeuropean} lists all individual languages in each group.
\begin{table}
  \centering
  \begin{tabular}{lrrcp{5cm}}
    \textbf{Languages} & \textbf{$\mu$} & \textbf{$\sigma$} & \textbf{Source} \\
    Gothic, Old West Norse, Icelandic, Faroese, Norwegian, Swedish, Danish, Old English, English, Frisian, Old High German, German, Luxembourgish, Swiss German, Dutch, Flemish and Afrikaans & Uniform between 2250~yBP and 20000~yBP & \parencite{chang2015ancestryconstrained} & Germanic  \\
    Latin, Nuorese, Cagliari, Romanian, Catalan, Portuguese, Spanish, French, Provencal, Walloon, Ladin, Romansh, Friulian and Italian & Uniform between 1750~yBP and 20000~yBP & \parencite{chang2015ancestryconstrained} & Romance  \\
    Old West Norse, Icelandic, Faroese, Norwegian, Swedish and Danish & Uniform between 1500~yBP and 20000~yBP & \parencite{chang2015ancestryconstrained} & Scandinavian  \\
    Czech, Slovak, Polish, Upper Sorbian, Ukrainian, Belarusian, Russian, Slovenian, Macedonian, Bulgarian, Serbian and Old Church Slavic & Uniform between 1500~yBP and 20000~yBP & \parencite{chang2015ancestryconstrained} & Slavic  \\
    Lithuanian and Latvian & Uniform between 1300~yBP and 20000~yBP & \parencite{chang2015ancestryconstrained} & East Baltic  \\
    Welsh, Breton and Cornish & Uniform between 1250~yBP and 20000~yBP & \parencite{chang2015ancestryconstrained} & British Celtic  \\
    Irish and Scots Gaelic & Uniform between 1050~yBP and 20000~yBP & \parencite{chang2015ancestryconstrained}  \\
    Tajik and Persian & Uniform between 750~yBP and 20000~yBP & \parencite{chang2015ancestryconstrained}  \\
    \textbf{Languages} & \textbf{$\mu$} & \textbf{$\sigma$} & \textbf{Source} \\
