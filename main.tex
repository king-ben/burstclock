...
\begin{figure}
  ...
  \caption{Runtimes of different analyses. Filled circles correspond to runs that reached an effective sample size of $200$ with $20\%$ burnin for all parameters. Empty circles correspond to runs which had an ESS of less than $200$ for at least one model parameter, and which were thus considered unconverged and not included in the analysis.}\label{f:runtimes}
\end{figure}
...
We therefore included in our statistics above those runs where all parameters had an effective sample size of 200 after a burn-in of $20\%$, instead of a burn-in of $10\%$.
...
behaviour. In contrast with some of our other analyses, this does not improve when adding the burst clock, either. Given the reported success
...
researchers to adapt, criticize, and modify our inference procedure, so that we can get towards comparable studies in
