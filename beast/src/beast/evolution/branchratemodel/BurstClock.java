...
public class BurstClock extends BranchRateModel.Base {
    ...
    public Input<RealParameter> persplitInput = new Input<>(
            "perSplit",
            "Effective number of changes after each split in the tree",
            Input.Validate.REQUIRED);

    public Input<BranchRateModel.Base> branchonlyClockInput = new Input<>(
            "branchonlyClock",
            "The clock model defining the rate along different branches (ignoring splits).",
            Input.Validate.XOR, meanRateInput);
    ...
    public void initAndValidate() {
        ...
        if (branchonlyClock == null) {
			...
			Log.err.println("WARNING: Explicit branchonly clock was specified for BurstClock " + getID() + " so mean rate input will be ignored.");
        }
    }
